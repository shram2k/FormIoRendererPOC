...
(function (c) {
    var b = { inEffect: { opacity: "show" }, inEffectDuration: 2000, stayTime: 2000, text: "", sticky: false, type: "notice", position: "middle-center", closeText: "", close: null };
    var a = { init: function (d) {
        ...
        c.extend(g, b, f);
        var j, e, d, i, h, k;
        k = (f.type == 'success') ? 'toast-box toast-green' : (f.type == 'error') ? 'toast-box toast-red' : 'toast-box toast-yellow'
        ...
        d = c("<div></div>").hide().addClass(k + " toast-type-" + g.type).appendTo(j).html(c("<p>").append(g.text)).animate(g.inEffect, g.inEffectDuration).wrap(e);
    ...
    , removeToast: function (e, d) {
        e.animate({ opacity: "0" }, 2000, function () {
            e.parent().animate({ height: "0px" }, 400, function () {
                e.parent().remove()
            })
        });
        if (d && d.close !== null) {
